...
(() => {
  ...
  const KONTAKT = { tel: '+381644500600', telPrikaz: '064/4500-600', wa: '381644500600', mejl: 'vilastevic@gmail.com' };
  ...
  const FORM_ENDPOINT = 'https://script.google.com/macros/s/AKfycbwM_zjT7NCVsRLQDV-AlOotI4jDktciNvrjENibk_jQEdDylf3Q0lm51V2Z__XaGjGy_A/exec';
  ...
  Object.keys(SLIKE).forEach(k => html.style.setProperty('--slika-' + k, 'url("' + SLIKE[k] + '")'));
  ...
  function postaviSliku(img) {
    ...
    img.addEventListener('error', () => {
      pokusaj += 1;
      if (pokusaj === 1 && prva !== mala) { img.src = mala; return; }
      img.style.visibility = 'hidden'; /* iza ostaje mirna siva ili tamna površina kontejnera */
    }, PASIVNO);
    img.addEventListener('load', () => img.classList.add('ucitana'), Object.assign({ once: true }, PASIVNO));
  ...
  qa('[data-podatak]').forEach(el => {
    ...
    if (v && String(v).trim()) { el.textContent = String(v).trim(); el.classList.remove('tbd'); }
  });
  ...
  const isoDatum = d => d.getFullYear() + '-' + pad(d.getMonth() + 1) + '-' + pad(d.getDate());
  const danas = () => isoDatum(new Date());
  const plusDana = (iso, n) => { const [g, m, d] = iso.split('-').map(Number); return isoDatum(new Date(g, m - 1, d + n)); };
  const lepDatum = iso => { if (!iso) return ''; const [g, m, d] = iso.split('-'); return d + '.' + m + '.' + g + '.'; };
  const kratakDatum = iso => { if (!iso) return ''; const [, m, d] = iso.split('-'); return d + '.' + m + '.'; };
  const jeDatum = v => /^\d{4}-\d{2}-\d{2}$/.test(v || '');
  const uOpsegu = (v, min, max) => { let n = parseInt(v, 10); if (isNaN(n)) n = min; return Math.min(max, Math.max(min, n)); };
  const mnozina = (n, jedan, dva, pet) => {
    const d = n % 10, s = n % 100;
    const rec = (d === 1 && s !== 11) ? jedan : (d >= 2 && d <= 4 && (s < 10 || s >= 20)) ? dva : pet;
    return n + ' ' + rec;
  };

  const trakaEl = q('#termin');
  const traka = trakaEl ? {
    el: trakaEl, forma: q('#forma-termin'),
    dolazak: q('#dolazak'), odlazak: q('#odlazak'), odrasli: q('#odrasli'), deca: q('#deca'),
    godine: q('#godine-dece'), godineNaslov: q('#godine-naslov')
  } : null;
  ...
  function stanjeIzAdrese() {
    ...
    const godine = (p.get('g') || '').split(',').filter(x => x !== '').slice(0, deca).map(x => String(uOpsegu(x, 0, 17)));
    return { dolazak, odlazak: odlazak > dolazak ? odlazak : (dolazak ? plusDana(dolazak, 1) : ''), odrasli: uOpsegu(p.get('odrasli') || 2, 1, 30), deca, godine };
  }
  function stanje() {
  ...
  const gostiTekst = s => mnozina(s.odrasli, 'odrasla osoba', 'odrasle osobe', 'odraslih') + (s.deca ? ' i ' + mnozina(s.deca, 'dete', 'deteta', 'dece') : '');
  const terminTekst = s => (s.dolazak && s.odlazak) ? lepDatum(s.dolazak) + ' do ' + lepDatum(s.odlazak) : '';

  /* Upit za prenos stanja na drugu stranicu. Prazan kad je sve podrazumevano. */
  function upitStanja(s) {
    const p = new URLSearchParams();
    if (s.dolazak && s.odlazak) { p.set('dolazak', s.dolazak); p.set('odlazak', s.odlazak); }
    if (s.odrasli !== 2) p.set('odrasli', String(s.odrasli));
    if (s.deca) { p.set('deca', String(s.deca)); const g = s.godine.slice(0, s.deca).filter(x => x !== ''); if (g.length) p.set('g', g.join(',')); }
    return p.toString();
  }
  ...
  function bookingUrl(opcije) {
    ...
    for (let i = 0; i < s.deca; i++) {
      const g = s.godine[i];
      url.searchParams.append('age', (g === undefined || g === '') ? '0' : String(g));
    }
  ...
  const waUrl = tekst => 'https://wa.me/' + KONTAKT.wa + '?text=' + encodeURIComponent(tekst);
  function porukaWa(jedinica, tema) {
    const s = stanje();
    const termin = terminTekst(s);
    const zaTermin = termin ? ' za termin ' + termin + ' (' + gostiTekst(s) + ')' : '';
    if (tema === 'spa') return 'Zdravo! Zanima me SPA u Vili Stević: slana soba, sauna, đakuzi i masaža.' + (termin ? ' Dolazimo ' + termin : '');
  ...
  function azurirajInterneLinkove(s) {
    ...
    interni.forEach(a => {
      ...
      a.setAttribute('href', strana + (upit ? '?' + upit : '') + (sidro !== undefined ? '#' + sidro : ''));
    });
  }
  function azurirajLinkove() {
    const s = stanje();
    const bu = bookingUrl();
    qa('a.js-booking').forEach(a => { a.href = bu; });
    qa('a.js-wa').forEach(a => { a.href = waUrl(porukaWa(a.dataset.jedinica, a.dataset.tema)); });
  ...
  function azurirajMini(s) {
    ...
    if (!t) return;
    t.textContent = (s.dolazak && s.odlazak)
      ? kratakDatum(s.dolazak) + ' do ' + kratakDatum(s.odlazak) + ' · ' + gostiTekst(s)
      : 'Termin nije izabran';
  }
  function prefilujKontakt(s) {
    [['k-dolazak', s.dolazak], ['k-odlazak', s.odlazak]].forEach(([id, v]) => {
      const i = q('#' + id);
      if (!i) return;
      if (!i.value || i.dataset.auto === '1') { i.value = v; i.dataset.auto = '1'; }
    });
  }
  qa('#k-dolazak, #k-odlazak').forEach(i => i.addEventListener('change', () => { i.dataset.auto = '0'; }, PASIVNO));
  ...
  function proveriGodine() {
    ...
    traka.godineNaslov.textContent = prazna.length ? 'Upišite godine dece:' : 'Godine dece:';
  ...
  function renderGodine(n, vrednosti) {
    ...
    for (let i = postojeca.length; i < n; i++) {
      ...
      p.innerHTML = '<label for="dete-' + (i + 1) + '">Dete ' + (i + 1) + '</label>' +
        '<input id="dete-' + (i + 1) + '" name="dete-' + (i + 1) + '" type="number" inputmode="numeric" min="0" max="17" placeholder="god.">';
      traka.godine.appendChild(p);
    }
    if (vrednosti) qa('input', traka.godine).forEach((i, k) => { if (vrednosti[k] !== undefined) i.value = vrednosti[k]; });
    traka.godine.classList.toggle('aktivno', n > 0);
    if (!n) { traka.godine.classList.remove('greska'); traka.godineNaslov.textContent = 'Godine dece:'; }
  }

  if (traka) {
    traka.dolazak.min = danas();
    traka.odlazak.min = plusDana(danas(), 1);
    traka.dolazak.addEventListener('change', () => {
      if (traka.dolazak.value) {
        ...
        if (!traka.odlazak.value || traka.odlazak.value < min) traka.odlazak.value = min;
      }
      azuriraj();
    }, PASIVNO);
    traka.odlazak.addEventListener('change', () => {
      const d = traka.dolazak.value;
      if (d && traka.odlazak.value && traka.odlazak.value <= d) traka.odlazak.value = plusDana(d, 1);
      azuriraj();
    }, PASIVNO);
    traka.odrasli.addEventListener('input', () => {
      if (traka.odrasli.value !== '') traka.odrasli.value = uOpsegu(traka.odrasli.value, 1, 30);
      azuriraj();
    }, PASIVNO);
    traka.odrasli.addEventListener('change', () => { traka.odrasli.value = uOpsegu(traka.odrasli.value, 1, 30); azuriraj(); }, PASIVNO);
    traka.deca.addEventListener('input', () => {
      const n = uOpsegu(traka.deca.value, 0, 10);
      if (traka.deca.value !== '') traka.deca.value = n;
      renderGodine(n);
      azuriraj();
    }, PASIVNO);
    traka.deca.addEventListener('change', () => { const n = uOpsegu(traka.deca.value, 0, 10); traka.deca.value = n; renderGodine(n); azuriraj(); }, PASIVNO);
    traka.godine.addEventListener('input', e => {
      if (e.target.value.trim() !== '') e.target.classList.remove('greska');
      if (!qa('input.greska', traka.godine).length) { traka.godine.classList.remove('greska'); traka.godineNaslov.textContent = 'Godine dece:'; }
      azuriraj();
    }, PASIVNO);
    if (traka.forma) traka.forma.addEventListener('submit', e => {
      e.preventDefault();
    ...
    if (iz.dolazak && iz.dolazak >= danas()) { traka.dolazak.value = iz.dolazak; traka.odlazak.min = plusDana(iz.dolazak, 1); traka.odlazak.value = iz.odlazak; }
    ...
    if (mini && 'IntersectionObserver' in window) {
      new IntersectionObserver(([en]) => {
        mini.classList.toggle('vidljiva', !en.isIntersecting && en.boundingClientRect.bottom < 48);
      }, { rootMargin: '-48px 0px 0px 0px', threshold: 0 }).observe(traka.el);
    }
  }

  /* Visina lepljive trake, za razmak pinovane SPA slike i za skok na sidra. */
  function izmeriTraku() {
    html.style.setProperty('--traka-visina', (traka && desktop() ? traka.el.offsetHeight : 0) + 'px');
  }
  izmeriTraku();
  if (traka && 'ResizeObserver' in window) new ResizeObserver(izmeriTraku).observe(traka.el);
  ...
  document.addEventListener('click', e => {
    ...
    if (b) {
      if (!proveriGodine()) { e.preventDefault(); return; }
      b.href = bookingUrl();
      return;
    }
  ...
  if (hamburger) {
    ...
    hamburger.addEventListener('click', () => {
      ...
      hamburger.setAttribute('aria-label', otvoren ? 'Zatvori meni' : 'Otvori meni');
    }, PASIVNO);
    qa('#meni a').forEach(a => a.addEventListener('click', zatvoriMeni, PASIVNO));
    document.addEventListener('keydown', e => { if (e.key === 'Escape') zatvoriMeni(); }, PASIVNO);
    window.matchMedia('(min-width: 768px)').addEventListener('change', e => { if (e.matches) zatvoriMeni(); });
  }
  ...
  if (pin && !smanjenPokret) {
    ...
    let aktivan = 0, zakazano = false;
    const izmeri = () => {
      zakazano = false;
      if (!desktop()) return;
      const centar = window.innerHeight / 2;
      let naj = 0, najD = Infinity;
      blokovi.forEach((b, i) => {
    ...
    const naSkrol = () => { if (!zakazano) { zakazano = true; requestAnimationFrame(izmeri); } };
    if ('IntersectionObserver' in window) {
      new IntersectionObserver(([en]) => {
        if (en.isIntersecting) { window.addEventListener('scroll', naSkrol, PASIVNO); naSkrol(); }
  ...
  /* --------------------------------------------------------------------------
     ...
     -------------------------------------------------------------------------- */
  const galerija = q('#galerija'), jos = q('#galerija-jos');
  if (galerija && jos) {
  ...
  function teloPoruke(forma) {
    ...
    if (vrsta === 'vaucer') {
      ...
      redovi.push('Upit za poklon vaučer sa sajta.', 'Ime: ' + v('ime'), 'Telefon: ' + v('telefon'), 'Mejl: ' + v('mejl'), 'Vaučer: ' + v('sadrzaj'));
    } else {
      const dolazak = v('dolazak'), odlazak = v('odlazak');
      const termin = (dolazak && odlazak) ? lepDatum(dolazak) + ' do ' + lepDatum(odlazak) : 'nije izabran';
      naslov = 'Upit sa sajta' + ((dolazak && odlazak) ? ': ' + termin : '');
      const godine = s.deca ? ' (godine: ' + s.godine.slice(0, s.deca).map(g => g || '?').join(', ') + ')' : '';
      redovi.push('Upit sa sajta.', 'Ime: ' + v('ime'), 'Telefon: ' + v('telefon'), 'Mejl: ' + v('mejl'), 'Termin: ' + termin, 'Odrasli: ' + s.odrasli + ', deca: ' + s.deca + godine);
    }
  ...
  function proveriFormu(forma) {
    ...
    const oznaci = ime => { const el = forma.elements[ime]; if (el && el.closest('.polje')) el.closest('.polje').classList.add('polje--greska'); };

    if (!v('ime')) { greske.push('Upišite ime.'); oznaci('ime'); }
    const tel = v('telefon'), mejl = v('mejl');
    if (!tel && !mejl) { greske.push('Upišite telefon ili mejl da bismo mogli da odgovorimo.'); oznaci('telefon'); oznaci('mejl'); }
    if (mejl && !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(mejl)) { greske.push('Mejl nije ispravan.'); oznaci('mejl'); }
    const d = v('dolazak'), o = v('odlazak');
    if (d && !jeDatum(d)) { greske.push('Datum dolaska nije ispravan.'); oznaci('dolazak'); }
    if (o && !jeDatum(o)) { greske.push('Datum odlaska nije ispravan.'); oznaci('odlazak'); }
    if (d && jeDatum(d) && d < danas()) { greske.push('Dolazak ne može biti u prošlosti.'); oznaci('dolazak'); }
    if (d && o && jeDatum(d) && jeDatum(o) && o <= d) { greske.push('Odlazak mora biti posle dolaska.'); oznaci('odlazak'); }
    if (forma.dataset.vrsta === 'vaucer' && !v('sadrzaj')) { greske.push('Upišite šta biste poklonili.'); oznaci('sadrzaj'); }
    if (forma.dataset.vrsta !== 'vaucer' && !v('poruka')) { greske.push('Napišite kratku poruku.'); oznaci('poruka'); }
    return greske;
  }
  ...
  function posaljiFormu(forma, kanal) {
    ...
    const rezervni = ' Ako vam se žuri, pozovite ' + KONTAKT.telPrikaz + ' ili pišite na WhatsApp.';

    if (kanal === 'wa') {
      const greske = proveriFormu(forma);
      if (greske.length) { javi(greske[0], 'greska'); return; }
      const prozor = window.open(waUrl(telo), '_blank');
      if (prozor) prozor.opener = null; else window.location.href = waUrl(telo);
    ...
    if (poljeVrednost(forma, 'website')) { javi('Upit je stigao. Javljamo se na telefon ili mejl koji ste ostavili.', 'uspeh'); forma.reset(); return; }
    ...
    polja.set('jedinica', forma.dataset.vrsta === 'vaucer' ? 'Poklon vaučer' : '');
    ...
    const vrati = () => { dugmad.forEach(b => { b.disabled = false; }); if (primarno) primarno.textContent = staroIme; };
    ...
    fetch(FORM_ENDPOINT, { method: 'POST', body: polja })
      ...
      .catch(() => { vrati(); mejlom('Slanje nije uspelo.'); });
  }

  qa('form.js-forma').forEach(forma => {
    let kanal = 'mejl';
    qa('button[data-kanal]', forma).forEach(b => b.addEventListener('click', () => { kanal = b.dataset.kanal; }, PASIVNO));
  ...
  if (svetlo && typeof svetlo.showModal === 'function') {
    const slikaEl = q('img', svetlo), opisEl = q('figcaption', svetlo);
    let grupa = [], indeks = 0;
    const prikazi = i => {
      ...
      slikaEl.alt = img.alt || '';
      opisEl.textContent = (img.alt ? img.alt + ' · ' : '') + (indeks + 1) + ' / ' + grupa.length;
    };
    const otvori = f => {
    ...
    svetlo.addEventListener('close', () => { html.classList.remove('svetlo-otvoreno'); slikaEl.removeAttribute('src'); }, PASIVNO);
    qa('figure[data-svetlo]').forEach(f => {
      f.setAttribute('tabindex', '0');
      f.setAttribute('role', 'button');
      f.addEventListener('click', () => otvori(f), PASIVNO);
      f.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); otvori(f); } });
    });
    q('.svetlo__zatvori', svetlo).addEventListener('click', () => svetlo.close(), PASIVNO);
    q('.svetlo__pre', svetlo).addEventListener('click', () => prikazi(indeks - 1), PASIVNO);
    q('.svetlo__sledeca', svetlo).addEventListener('click', () => prikazi(indeks + 1), PASIVNO);
    svetlo.addEventListener('click', e => { if (e.target === svetlo) svetlo.close(); }, PASIVNO);
    svetlo.addEventListener('keydown', e => { if (e.key === 'ArrowRight') prikazi(indeks + 1); if (e.key === 'ArrowLeft') prikazi(indeks - 1); }, PASIVNO);
  }
